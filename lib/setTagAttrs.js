...
var $,
    ...
    applyStylesAsProps = function ($el, style, attribute) {
        ...
        if (styleVal !== undefined && attribute !== null) {
            ...
            styleVal = styleVal.replace( /^(\d+)px/i, "$1" ) // remove unit if pixels
    ...
    loopThroughMap = function (index, el) {
        ...
        if (isTable === true && $el[0].name === 'table') { // we're attempting to stay backward compatible with setTableAttrs
            resetAttr($el, 'border');
